  ...
  var A_DAY_IN_MILLISECONDS = 86400000
    ...
    , generateMonthAndYearHeader = function generateMonthAndYearHeader(prevButton, nextButton, preventMobile) {
      ...
      if (isMobile) {
        ...
        return [
          '<div class="_lf-datepicker-calendar-header">',
            '<div class="_lf-datepicker-calendar-header-middle _lf-datepicker-mobile-item _lf-datepicker-calendar-month">',
      ...
      return [
        '<div class="_lf-datepicker-calendar-header">',
          '<div class="_lf-datepicker-calendar-header-left">',
            '<a class="_lf-datepicker-calendar-month-button" href="javascript:void(0)" ng-class="{\'_lf-datepicker-item-hidden\': !willPrevMonthBeSelectable()}" ng-click="prevMonth()" title="{{ buttonPrevTitle }}">',
              prevButton,
            '</a>',
          '</div>',
          '<div class="_lf-datepicker-calendar-header-middle _lf-datepicker-calendar-month">',
            '{{month}}&nbsp;',
            '<a href="javascript:void(0)" ng-click="paginateYears(year); showYearsPagination = !showYearsPagination;">',
              '<span>',
                '{{year}}',
                '<i ng-class="{\'_lf-datepicker-calendar-header-closed-pagination\': !showYearsPagination, \'_lf-datepicker-calendar-header-opened-pagination\': showYearsPagination}"></i>',
              '</span>',
            '</a>',
          '</div>',
          '<div class="_lf-datepicker-calendar-header-right">',
          '<a class="_lf-datepicker-calendar-month-button" ng-class="{\'_lf-datepicker-item-hidden\': !willNextMonthBeSelectable()}" href="javascript:void(0)" ng-click="nextMonth()" title="{{ buttonNextTitle }}">',
            nextButton,
          '</a>',
          '</div>',
        '</div>'
    ...
    , generateYearsPaginationHeader = function generateYearsPaginationHeader(prevButton, nextButton) {
      ...
      return [
        '<div class="_lf-datepicker-calendar-header" ng-show="showYearsPagination">',
          '<div class="_lf-datepicker-calendar-years-pagination">',
            '<a ng-class="{\'_lf-datepicker-active\': y === year, \'_lf-datepicker-disabled\': !isSelectableMaxYear(y) || !isSelectableMinYear(y)}" href="javascript:void(0)" ng-click="setNewYear(y)" ng-repeat="y in paginationYears track by $index">',
              '{{y}}',
            '</a>',
          '</div>',
          '<div class="_lf-datepicker-calendar-years-pagination-pages">',
            '<a href="javascript:void(0)" ng-click="paginateYears(paginationYears[0])" ng-class="{\'_lf-datepicker-item-hidden\': paginationYearsPrevDisabled}">',
              prevButton,
            '</a>',
            '<a href="javascript:void(0)" ng-click="paginateYears(paginationYears[paginationYears.length -1 ])" ng-class="{\'_lf-datepicker-item-hidden\': paginationYearsNextDisabled}">',
              nextButton,
            '</a>',
          '</div>',
        '</div>'
    ...
    , generateDaysColumns = function generateDaysColumns() {
      ...
      return [
      '<div class="_lf-datepicker-calendar-days-header">',
    ...
    , generateDays = function generateDays() {
      ...
      return [
        '<div class="_lf-datepicker-calendar-body">',
          '<a href="javascript:void(0)" ng-repeat="px in prevMonthDays" class="_lf-datepicker-calendar-day _lf-datepicker-disabled">',
            '{{px}}',
          '</a>',
          '<a href="javascript:void(0)" ng-repeat="item in days" ng-click="setDatepickerDay(item)" ng-class="{\'_lf-datepicker-active\': selectedDay === item && selectedMonth === monthNumber && selectedYear === year, \'_lf-datepicker-disabled\': !isSelectableMinDate(year + \'/\' + monthNumber + \'/\' + item ) || !isSelectableMaxDate(year + \'/\' + monthNumber + \'/\' + item) || !isSelectableDate(monthNumber, year, item) || !isSelectableDay(monthNumber, year, item),\'_lf-datepicker-today\': item === today.getDate() && monthNumber === (today.getMonth() + 1) && year === today.getFullYear() && !selectedDay}" class="_lf-datepicker-calendar-day">',
            '{{item}}',
          '</a>',
          '<a href="javascript:void(0)" ng-repeat="nx in nextMonthDays" class="_lf-datepicker-calendar-day _lf-datepicker-disabled">',
            '{{nx}}',
          '</a>',
        '</div>'
      ];
    ...
    , generateHtmlTemplate = function generateHtmlTemplate(prevButton, nextButton, preventMobile) {
      ...
      var toReturn = [
        '<div class="_lf-datepicker-calendar {{datepickerClass}} {{datepickerID}}" ng-class="{\'_lf-datepicker-forced-to-open\': checkVisibility()}" ng-blur="hideCalendar()">',
        '</div>'
      ]
    ...
    , datepickerDirective = function datepickerDirective($window, $compile, $locale, $filter, $interpolate, $timeout) {
      ...
      var linkingFunction = function linkingFunction($scope, element, attr) {
        ...
        var selector = attr.selector
          ...
          , theCalendar
          , defaultPrevButton = '<b class="_lf-datepicker-default-button">&lang;</b>'
          , defaultNextButton = '<b class="_lf-datepicker-default-button">&rang;</b>'
          ...
          , showCalendar = function showCalendar() {
            ...
            pageDatepickers = $window.document.getElementsByClassName('_lf-datepicker-calendar');

            angular.forEach(pageDatepickers, function forEachDatepickerPages(value, key) {
              if (pageDatepickers[key].classList) {

                pageDatepickers[key].classList.remove('_lf-datepicker-open');
              } else {

                classHelper.remove(pageDatepickers[key], '_lf-datepicker-open');
              }
            });

            if (theCalendar.classList) {

              theCalendar.classList.add('_lf-datepicker-open');
              if (dateFormat) {
              ...
              classHelper.add(theCalendar, '_lf-datepicker-open');
            }
        ...
        $scope.hideCalendar = function hideCalendar() {
          if (theCalendar.classList) {
            theCalendar.classList.remove('_lf-datepicker-open');
          } else {

            classHelper.remove(theCalendar, '_lf-datepicker-open');
          }
        };
        ...
        if ($scope.datepickerAppendTo &&
          ...
          theCalendar = element[0].querySelector('._lf-datepicker-calendar');
        }
